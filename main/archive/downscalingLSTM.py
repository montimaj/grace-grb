# LSTM-based TWS Downscaling (2002–2024)
...
# Define LSTM model
class LSTMDownscaler(nn.Module):
...
model = LSTMDownscaler(input_size=len(predictors))
...
pred_df.to_csv("figures/Downscaled_TWSA_Daily_2002_2024.csv", index=False)
...
plt.title("Simulated Feature Importance for LSTM Downscaling")
plt.tight_layout()
plt.savefig("figures/simulated_feature_importance_LSTM.png", dpi=300)
plt.close()
...
plt.xlabel("Date")
plt.ylabel("TWSA")
plt.title("Actual vs Predicted TWSA (2002–2024)")
plt.legend()
plt.grid(True)
plt.tight_layout()
plt.savefig("figures/temporal_plot_TWSA.png", dpi=300)
plt.close()
...
plt.title("Residuals Plot (Actual - Predicted TWSA)")
plt.xlabel("Date")
plt.ylabel("Residual")
plt.grid(True)
plt.tight_layout()
plt.savefig("figures/residuals_plot_TWSA.png", dpi=300)
plt.close()
...
plt.title("Scatter Plot: Actual vs Predicted TWSA")
plt.grid(True)
plt.tight_layout()
plt.savefig("figures/scatter_plot_TWSA.png", dpi=300)
plt.close()
