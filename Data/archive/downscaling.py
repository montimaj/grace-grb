# BiLSTM+Attention-based TWS Downscaling (2002-2024)
...
class BiLSTMAttentionDownscaler(nn.Module):
...
model = BiLSTMAttentionDownscaler(input_size=len(predictors))
...
with torch.no_grad():
    ...
    attention_weights = model.attn_weights.squeeze().numpy()
...
pred_df.to_csv("figures/Downscaled_TWSA_Daily_2002_2024_BiLSTM_Att.csv", index=False)
...
plt.xlabel("Date")
plt.ylabel("TWSA")
plt.title("Actual vs Predicted TWSA (2002–2024) [BiLSTM+Attention]")
plt.legend()
plt.grid(True)
plt.tight_layout()
plt.savefig("figures/temporal_plot_TWSA_BiLSTM_Att.png", dpi=300)
plt.close()
...
plt.title("Residuals Plot (Actual - Predicted TWSA) [BiLSTM+Attention]")
plt.xlabel("Date")
plt.ylabel("Residual")
plt.grid(True)
plt.tight_layout()
plt.savefig("figures/residuals_plot_TWSA_BiLSTM_Att.png", dpi=300)
plt.close()
...
plt.title("Scatter Plot: Actual vs Predicted TWSA [BiLSTM+Attention]")
plt.grid(True)
plt.tight_layout()
plt.savefig("figures/scatter_plot_TWSA_BiLSTM_Att.png", dpi=300)
plt.close()

plt.figure(figsize=(8, 4))
plt.plot(train_losses, marker='o')
plt.title("Training Loss Curve [BiLSTM+Attention]")
plt.xlabel("Epoch")
plt.ylabel("Huber Loss")
plt.grid(True)
plt.tight_layout()
plt.savefig("figures/training_loss_curve_BiLSTM_Att.png", dpi=300)
plt.close()
...
plt.title("Simulated Feature Importance for BiLSTM+Attention Downscaling")
plt.tight_layout()
plt.savefig("figures/simulated_feature_importance_BiLSTM_Att.png", dpi=300)
plt.close()
